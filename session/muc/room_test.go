...
func (s *MucSuite) Test_Room_StatusConnected(c *C) {
	r := NewRoom(jid.ParseBare("foo@bar.com"))

	selfOccupantConnected := make(chan bool)

	r.Subscribe(func(ev events.MUC) {
		switch ev.(type) {
		case events.MUCSelfOccupantConnected:
			selfOccupantConnected <- true
		}
	})

	go r.StatusConnected()

	ok := <-selfOccupantConnected
	c.Assert(ok, Equals, true)
}

func (s *MucSuite) Test_Room_StatusDisconnected(c *C) {
	r := NewRoom(jid.ParseBare("foo@bar.com"))
	...
	c.Assert(r.Roster().UpdateOrAddOccupant(op), Equals, false)

	occ, _ := r.Roster().GetOccupant(op.Nickname)
	r.AddSelfOccupant(occ)

	selfOccupantDisconnected := make(chan bool)

	r.Subscribe(func(ev events.MUC) {
		switch ev.(type) {
		case events.MUCSelfOccupantDisconnected:
			selfOccupantDisconnected <- true
		}
	})

	go r.StatusDisconnected()

	ok := <-selfOccupantDisconnected
	c.Assert(ok, Equals, true)

	_, ok = r.Roster().GetOccupant(op.Nickname)
